"""Optional SQLite-backed lexicon store.

Uses only the stdlib ``sqlite3`` module — no additional dependencies required.

Schema
------
The database is expected to have the following tables (created automatically
when using :meth:`SqliteLexiconStore.from_builtin_resources`):

- ``lexicon_syllables``        (base, surface, freq)
- ``lexicon_words``           (surface, type, freq, domain)
- ``lexicon_units``           (surface, freq, domain)
- ``lexicon_abbreviations``   (abbreviation, normalized, expansions, ambiguous, tags)
- ``lexicon_foreign_words``   (word)
- ``lexicon_phrases``         (phrase, normalized, no_tone, n, freq, domain, tags)
- ``lexicon_ocr_confusions``  (noisy, normalized_noisy, corrections, confidence)

Array-typed columns (``expansions``, ``tags``, ``corrections``) are stored as
JSON text.
"""
...
_COLUMN_HINT = "Type: SqliteLexiconStore"
...
class SqliteLexiconStore(LexiconStore):
    ...
    Args:
        db_path: Filesystem path to the SQLite database file.  The database
            must already exist with the schema documented in this module.
    ...
    def from_path(cls, db_path: str | Path) -> SqliteLexiconStore:
        ...
        The database must already contain the tables documented in this module.
        """
        return cls(db_path)
    ...
    def _query_word_no_tone(self, key: str) -> list[LexiconEntry]:
        ...
        # Accent-insensitive word lookup requires fetching and filtering in Python
        # since we can't easily match no_tone without a stored column.
        # For large databases, users should add a ``no_tone`` column to the schema.
